...
public partial class Day08 : Advent.Day
{
    ...
    public override void DoWork()
    {
        ...
        foreach (string line in Inputs)
        {
            ...
            Dictionary<char,char> translations;

            if (Part1)
            {
                result += outputs.Count(o => o.Length == 2 || o.Length == 3 || o.Length == 4 || o.Length == 7);
            }
            else
            {
    ...
    private static Dictionary<char, char> GetTranslations(string[] inputs)
    {
        ...
        interim[4] = SubtractStrings(commonLetters[7], [commonLetters[6], commonLetters[4]]);
        interim[3] = SubtractStrings(commonLetters[7], [interim[4], commonLetters[6], commonLetters[2]]);
        interim[6] = SubtractStrings(commonLetters[5], [interim[0], interim[3]]);
        interim[1] = SubtractStrings(commonLetters[4], [commonLetters[2], interim[3]]);
        interim[5] = SubtractStrings(commonLetters[6], [interim[0], interim[1], interim[6]]);
    ...
    private static string SubtractString(string input, string toRemove) => SubtractStrings(input, [toRemove]);
    ...
    private int Translate(string input, Dictionary<char, char> translations)
    {
        ...
        foreach(char c in input)
            translated.Append(translations[c]);
        return values[SortString(translated.ToString())];
    }
