...
public partial class Day08 : Advent.Day
{
    public override void DoWork()
    {
        ...
        Dictionary<(int, int), TreeStats> trees = [];
        ...
        int visibleTrees = trees.Count(t => t.Value.Visible);
        int maxScore = trees.Max(t => t.Value.Score);
        Output = (Part1 ? visibleTrees : maxScore).ToString();
    }
    private sealed class TreeStats(int height)
    {
    ...
    private static void GetStats(int x, int y, int height, int width, Dictionary<(int, int), TreeStats> trees)
    {
