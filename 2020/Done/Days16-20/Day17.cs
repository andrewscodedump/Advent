        ...
        for (int c = 1; c <= cycles; c++)
        {
            ...
            for (int x = -c; x <= c + startWidth - 1; x++)
                for (int y = -c; y <= c + startWidth - 1; y++)
                    for (int z = -c; z <= c; z++)
                        for (int w = WhichPart == 1 ? 0 : -c; w <= (WhichPart == 1 ? 0 : c); w++)
                        {
                            int neighbourCount = WhichPart == 1 ? neighbours4D.Where(n => n.dw == 0).Where(n => map[(x + n.dx, y + n.dy, z + n.dz, 0)] == 1).Count() : neighbours4D.Where(n => map[(x + n.dx, y + n.dy, z + n.dz, w + n.dw)] == 1).Count();
